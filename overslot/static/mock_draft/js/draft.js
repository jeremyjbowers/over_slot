...
(function () {
  ...
  function upperBoundSigningCostForFilter(player, pick, slotValue, teamName) {
    ...
    if (player.class === 'H') return player.cost;
    if (player.class !== 'C') {
      return player.cost;
  ...
  function firstAffordableUndrafted(team, pickIndex, slotValue) {
    ...
    const affordable = byCost.filter(
      p => upperBoundSigningCostForFilter(p, pick, slotValue, team) <= maxSpend
    );
    ...
    function renderList(filter) {
      ...
      if (q) {
        ...
        if (labelEl) {
          ...
          labelEl.title =
            state.topViewMode === 'highestRanked'
              ...
              : 'Affordable players for this pick’s Max only — no undraftable fill-ins. Pool tight vs slot? Random senior sign is listed first when it fits.';
        }
        if (!fullBoard.length) {
        ...
        if (state.topViewMode === 'highestRanked') {
          ...
          const tightPool = pick.value > 0 && maxThisPick < pick.value;
          const seniorPlayer = makeRandomSeniorSignPlayer(pickIdx);
          const seniorSig = signability(seniorPlayer);
          ...
          if (tightPool && seniorSig.signable) {
            headEntries.unshift({
              player: seniorPlayer,
              signable: true,
              reason:
                'Pool-tight vs this slot — senior sign fits Max while holding $150k × each pick still to come.'
            });
          }

          const chalk = headEntries.find(e => e.signable)?.player || null;
          if (labelEl && headEntries.length === 0 && fullBoard.length) {
            labelEl.textContent = 'No one fits this pick’s Max — try Ranked or search';
          }
